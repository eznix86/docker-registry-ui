<template>
	<Dialog :model-value="isOpen" @update:model-value="close">
		...
		<p class="text-muted-foreground mb-4">
			Select tags to delete. This cannot be undone.
		</p>
		...
		<div v-if="isDeleting" class="border border-outline rounded-lg p-4 text-sm text-muted-foreground mb-6">
			Loading all tags...
		</div>
		<div v-else-if="tags.length === 0" class="border border-outline rounded-lg p-4 text-sm text-muted-foreground mb-6">
			No tags available.
		</div>
		...
		<p v-if="deleteError" class="text-red-500 mb-4">
			{{ deleteError }}
		</p>
...
function open() {
	...
	selectedTags.value = []
	deleteError.value = null
	router.reload({ only: ["bulkDeleteTags"] })
}
...
function toggleAll(v: any) {
	selectedTags.value = v ? props.tags.map(t => t.name) : []
}
...
async function confirmDelete() {
	...
	try {
		...
		if (result.failed > 0) {
			deleteError.value = `Deleted ${result.deleted}, failed ${result.failed}`
			return
		}
...
function calculateTagSize(tag: Tag): string {
	return formatBytes(tag.images.reduce((s, i) => s + i.size, 0))
}
