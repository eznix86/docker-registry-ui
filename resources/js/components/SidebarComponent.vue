...
<template>
	<aside class="w-80 bg-background h-full border-r border-outline p-8 flex flex-col">
		<div class="flex-1 overflow-y pr-2 -mr-2">
			...
			<div class="mb-6">
				<div class="block">
					<span id="architectures-label" class="text-sm font-semibold text-foreground mb-2 block">Architectures</span>
			...
			<Checkbox id="show-untagged" :model-value="showUntagged" label="Show untagged repositories" label-variant="default" @update:model-value="toggleUntagged" />
		</div>

		<Button variant="ghost" class="flex items-center gap-3 w-full px-2 py-2 mt-auto justify-start" @click="isSettingsOpen = true">
...
const selectedRegistries = computed(() => (filters.value as any).registries || [])
const showUntagged = computed(() => Boolean((filters.value as any).showUntagged))

const selectedArchitecture = computed(() => {
	const archs = (filters.value as any).architectures || []
	return archs.length > 0 ? archs[0] : "all"
})
