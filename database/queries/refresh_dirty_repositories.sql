...
-- Incremental refresh of repository_stats for dirty repositories only
WITH dirty AS (
    SELECT repo_id FROM dirty_repos
),
unique_manifests_per_repo AS (
    SELECT DISTINCT
        tags.repo_id,
...
tag_counts AS (
    SELECT tags.repo_id, COUNT(*) as count
    FROM tags
    INNER JOIN dirty ON tags.repo_id = dirty.repo_id
    GROUP BY tags.repo_id
)
...
SELECT
    ...
    CURRENT_TIMESTAMP
FROM repositories
INNER JOIN dirty ON repositories.id = dirty.repo_id
