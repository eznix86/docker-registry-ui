...
-- Incremental refresh of tag_details for dirty tags only
WITH dirty AS (
    SELECT tag_id FROM dirty_tags
),
unique_manifests_per_tag AS (
    SELECT DISTINCT
        tags.id as tag_id,
    ...
    FROM tags
    INNER JOIN dirty ON tags.id = dirty.tag_id
