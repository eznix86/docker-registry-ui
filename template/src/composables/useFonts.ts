...
const currentFontSans = ref<FontSans>(
	(localStorage.getItem(STORAGE_KEY_SANS) as FontSans) || "roboto",
)
const currentFontMono = ref<FontMono>(
	(localStorage.getItem(STORAGE_KEY_MONO) as FontMono) || "roboto-mono",
)
...
applyFonts(currentFontSans.value, currentFontMono.value)
...
export function useFonts() {
	...
	const getFontSansLabel = (font: FontSans): string => {
		return (
			FONT_SANS_OPTIONS.find(f => f.value === font)?.label || font
		)
	}

	const getFontMonoLabel = (font: FontMono): string => {
		return (
			FONT_MONO_OPTIONS.find(f => f.value === font)?.label || font
		)
	}
